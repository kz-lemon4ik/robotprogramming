 ...
 public:
  IKSolverNode() : ik_calculator_(1.0, 1.0) {
    ...
    if (link1_length_ <= 0.0 || link2_length_ <= 0.0) {
      ROS_ERROR("Invalid link lengths: L1=%.2f, L2=%.2f. Must be positive.", link1_length_,
                link2_length_);
      ros::shutdown();
      return;
    }
    ...
    target_sub_ = nh_.subscribe("target_goal", 10, &IKSolverNode::targetCallback, this);
    ...
    ROS_INFO("Workspace: min_reach=%.2f, max_reach=%.2f", ik_calculator_.get_min_reach(),
             ik_calculator_.get_max_reach());
  ...
  void targetCallback(const geometry_msgs::Point::ConstPtr& msg) {
    ...
    if (success) {
      ROS_INFO("IK solution: theta1=%.3f rad (%.1f deg), theta2=%.3f rad (%.1f deg)", theta1,
               theta1 * 180.0 / M_PI, theta2, theta2 * 180.0 / M_PI);
