...
void print_test_result(const robotprogramming::IKCalculator& calculator,
    ...
    if (success) {
        ...
        std::cout << "theta1: " << std::fixed << std::setprecision(4) 
                  << theta1 << " rad (" << theta1 * 180.0 / M_PI << " deg)" << std::endl;
        std::cout << "theta2: " << std::fixed << std::setprecision(4) 
                  << theta2 << " rad (" << theta2 * 180.0 / M_PI << " deg)" << std::endl;
        ...
        std::cout << "Forward check: (" << std::fixed << std::setprecision(4) 
                  << calc_x << ", " << calc_y << ")" << std::endl;
        
        double error = std::sqrt((calc_x - test.x) * (calc_x - test.x) + 
                                (calc_y - test.y) * (calc_y - test.y));
...
int main() {
    ...
    std::vector<TestCase> test_cases = {
        {1.5, 0.0, "Basic horizontal position"},
